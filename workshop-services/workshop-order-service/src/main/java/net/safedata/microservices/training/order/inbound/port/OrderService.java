...
public class OrderService {
    ...
    @Transactional
    public void updateOrder(Long orderId, final OrderDTO orderDTO) {
        final long customerId = orderDTO.getCustomerId();
        Order order = persistenceOutboundPort.findOrder(orderId);
        order.setStatus(OrderStatus.ORDER_UPDATE_IN_PROCESS);
        persistenceOutboundPort.save(order);

        LOGGER.info("Updating an order for the customer with the ID {}, for {} items...", customerId,
                orderDTO.getOrderItems().size());
        final double orderTotal = orderDTO.getOrderTotal();

        CompletableFuture.runAsync(() -> publishChargeOrder(customerId, orderId, orderTotal))
                .thenRunAsync(() -> publishOrderUpdatedEvent(customerId, orderId));
    }
    ...
    private void publishOrderUpdatedEvent(final long customerId, final long orderId) {
        messagingOutboundPort.publishOrderUpdatedEvent(
                new OrderUpdatedEvent(getNextMessageId(), getNextEventId(), customerId, orderId));
    }
